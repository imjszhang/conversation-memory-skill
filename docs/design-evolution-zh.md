# Skill is All you Need：为 Claude 构建记忆系统
...
在构建 V2 的过程中，我有了另一个从根本上影响最终设计的领悟：

### 记忆应该是自动的，而非命令式的

最初的设计有多个命令：`save_memory`、`activate_memory`、`archive_old_memories`。但这感觉不对。

**人类记忆不是这样工作的。** 你不会有意识地决定"把这个存到长期记忆"或"归档那个旧记忆"。它自然而然地发生。

洞见：**一个好的记忆系统应该只需要一个动作——保存。** 其他一切都应该是自动的或不必要的。

### 拥抱冗余

我注意到的另一个反模式是试图避免重复记忆。但同样，这不是人类记忆的工作方式。

想想看：
- 你可能从多次对话中记住同一件事
- 每个记忆有略微不同的上下文和重点
- 这种冗余实际上有助于回忆——更多路径通向同一信息

**冗余是特性，不是缺陷。** 不要去重。不要合并。只管保存。

### 简化工作流

基于这些洞见，我移除了：
- `activate_memory.js` - 不必要的复杂性
- `archive_old_memories.js` - 让记忆自然积累

剩下的是：
- **一个保存动作** - 这就是你所需要的全部
- **基于关键词的召回** - 自然触发，就像一个词可以唤起记忆
- **对不完美的容忍** - 足够好的摘要效果出奇地好

这模仿了人类记忆实际的工作方式：**自动捕获、关联召回、优雅降级。**
...
安装非常简单——只需将 skill 文件夹复制到你项目的 `.claude/skills/` 目录下即可开始使用。

欢迎贡献代码、提交 Issue 和反馈意见！

---

*这段设计之旅表明，构建有效工具不仅需要编码技能，还需要深入理解平台并基于真实反馈进行迭代。有时候最好的解决方案不是添加复杂性，而是认识并使用已有的东西。*
